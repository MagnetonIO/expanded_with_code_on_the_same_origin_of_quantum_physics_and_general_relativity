...
-- A mock $(∞,1)$-category for spacetime and quantum states:
...
data ChainComplex a = ChainComplex [a]
...
evolveState (State psi) dt curvature =
  let h = constructHamiltonian dt curvature
      phase = h LA.! (0,0) -- take a component as a simple factor
...
simulateEvolution initialState times curvatureFunc =
  scanl (\st t -> evolveState st t (curvatureFunc t)) initialState times
...
runUnifiedSimulation = do
  ...
  -- Spectral decomposition of a sample Hamiltonian:
